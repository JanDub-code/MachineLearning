...
"""
FÁZE 1: Stažení Cenových Dat (10 let)
=====================================

Tento skript stahuje denní data pro S&P 500 společnosti,
agreguje je na měsíční a počítá technické indikátory.

Výstup: ../data_10y/all_sectors_full_10y.csv
"""
...
END_DATE = "2025-10-01"
DATA_DIR = "../data_10y"  # Upraveno pro strukturu CleanSolution

# PRO TESTOVÁNÍ: Počet firem na sektor (None = všechny)
TEST_LIMIT = None  # None = plný run (50 firem/sektor)
...
def main():
    log("=" * 80)
    log("FÁZE 1: STAŽENÍ CENOVÝCH DAT (10 LET)")
    log("=" * 80)
    ...
    if TEST_LIMIT:
        log(f"⚠ TESTOVACÍ REŽIM: {TEST_LIMIT} firem/sektor")
    else:
        log("🚀 PLNÝ RUN: všechny firmy")
    ...
    log(f"📊 Výstupní složka: {DATA_DIR}/")
    
    log("\n" + "="*80)
    log("Další krok: python scripts/1_download_fundamentals.py")
    log("="*80)
